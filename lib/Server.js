...
class Server
{
    ...
    onMessage(message, webSocket)
    {
        ...
        if(msg.action === 'subscribe')
        {
            ...
            {
                events = channel.constructor.events.map((e) => e.name());
            } else {
                events = [msg.event];
            }
            ...
            events.forEach(event => {
                ...
                const subscription = this.subscriptionManager.subscribe(
                    webSocket.__pascal.uuid, msg.channel, event, msg.snapshot, msg.filters || []
                );

                const subscribedEvent = new SubscriptionSubscribedEvent(subscription.serialize());
