...
class Listener
{
     ...
     */
    listen(milliSeconds) {
        this.interval = setInterval(this.tick.bind(this), milliSeconds);
    }
    ...
    async tick()
    {
        ...
        if(this.blockChain.latestBlock.block !== lastMinedBlock) {
            ...
            for(let o = 0; o < minedOps.length; o++) {
                minedOps[o] = new Operation(minedOps[o]);
                await minedOps[o].init(async (payload) => {
                    return await this.pascalRPCClient.payloadDecrypt(payload);
                }, async (publicKey) => {
                    return await this.pascalRPCClient.decodePublicKey(publicKey);
                });

                for (let [, account] of minedOps[o].accounts.entries()) {
                    this.blockChain.addAccount(await this.pascalRPCClient.getAccount(account));
                }
            }

            this.blockChain.addBlock(await this.pascalRPCClient.getBlock(lastMinedBlock), minedOps);
        }
        ...
        for(let o = 0; o < pendingOperations.length; o++)
        {
            ...
            const pendingOp = new Operation(pendingOperations[o]);
            await pendingOp.init(async (payload) => {
                return await this.pascalRPCClient.payloadDecrypt(payload);
            }, async (publicKey) => {
                return await this.pascalRPCClient.decodePublicKey(publicKey);
            });
            ...
            this.blockChain.addOperation(pendingOp);
        }
