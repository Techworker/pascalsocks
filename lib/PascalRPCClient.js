...
class PascalRPCClient
{
    ...
    async findOperation(ophash) {
        ...
        const operation = new Operation(response.result);
    ...
    async getLastBlocks(num)
    {
        ...
        return response.result.map(block => new Block(block));
    }
    ...
    {
        const response = await this.client.request('getaccount', {account: account.account});
        if(this.handleOnError(response)) {
            return [];
        }
    ...
    async getBlock(block)
    {
        ...
        return new Block(response.result);
    }
